...
\begin{description}
	\item[L'équipe responsable des serveurs dédiées vserveurs \index{VServer}\footnotemark]
...
\paragraph*{}
Dans ce document, nous appelerons les systèmes d'exploitations virtualisés/invités \textbf{VM} comme \emph{Virtual Machine}\index{Machine Virtuelle (VM)} ou \textbf{domU}\index{domU} suivant le contexte.
Le système d'exploitation principal/hôte sera appelé dom0\index{dom0} ou hyperviseur si aucun autre logiciel n'a déjà ce role dans l'architecture de virtualisation
utilisée.

\paragraph*{}
\begin{description}

	\item[Par émulation] Fig \index{Émulation matérielle}\ref{emulation} : Relativement facile à mettre en oeuvre, compatible avec tout les systèmes d'exploitations et toutes les architectures
	de processeur.\\
...
\item 	AWH s'occupe d'acheter, de configurer et de brancher en Data Center\index{Data Center}\footnote{un Data Center, souvent abbrégé en DC, est une salle
...
\subsection{Le cloud appliqué à l'hébergement de services ou d'OS\index{OS}\protect\footnote{De l'anglais \emph{Operating System} : Système d'exploitation}}
\paragraph*{}
...
\begin{listi}
	\item Créer et supprimer des VMs sans se soucier de l'endroit exacte ou elles seront réellement éxécutées (principe de base Cloud).
	...
	\item Éffectuer des \index{\emph{Live Migration}}\emph{live migration}. C'est à dire déplacer une VM d'un hyperviseur à un autre sans interruption de service.\\
...
\begin{description}
	\item[OpenStack] \index{OpenStack} est un logiciel open source créée par Rackspace et la NASA. Plus de 120 compagnies ont joint le projet dont Citrix System, Dell, AMD, Intel,
	...
	\item[OpenNebula] \index{OpenNebula} est un autre logiciel open source principalement financé par RESERVOIR
...
\paragraph*{}
Mon sujet de stage était d'étudier le logiciel OpenNebula et de le modifier afin d'ajouter le \emph{scale-in} et une gestion plus souple des images disque.
